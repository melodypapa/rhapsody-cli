# py_rhapsody: Python COM API for IBM Rhapsody — Design
...
Provide `py_rhapsody`, a Pythonic object-oriented wrapper around the Rhapsody
COM API that:
...
Importable package: `py_rhapsody` (distinct from the repo name `py-rhapsody`).

### Connection layer (`py_rhapsody/application.py`)
...
### Wrapping pattern (`py_rhapsody/_core.py`)
...
### Error handling (`py_rhapsody/exceptions.py`)
